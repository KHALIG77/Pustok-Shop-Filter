...
namespace PustokStart.Controllers
{
    public class BookController:Controller
    {
		...
		public IActionResult Detail(int id,int commentcount)
		{
			commentcount=commentcount+2;
			Book book = _context.Books
				.Include(x => x.BookImages)
				.Include(x => x.Author)
				.Include(x => x.Genre).Include(x=>x.BookComments.Take(commentcount)).ThenInclude(x=>x.AppUser).Include(x => x.Tags).ThenInclude(t => t.Tag).FirstOrDefault(x=>x.Id == id);
			int totalCommentCount=_context.BookComments.Where(c=>c.BookId==id).Select(c=>c.BookId==id).ToList().Count;
			...
			if (commentcount > 2)
			{
				return PartialView("_CommentReviewPartialView",vm);
